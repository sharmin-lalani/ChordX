...
public class ChordNodeImpl extends UnicastRemoteObject implements ChordNode{
	...
	public ChordNode cn;
	public FingerTableEntry[] fingertable = null;
	public HashMap<Integer, HashMap<String, String>> data = new HashMap<Integer, HashMap<String, String>>();
	...
	public NodeInfo closest_preceding_finger(int id) {
		...
		// Binary search on finger table
		
		/*
		if((myID >= midID && (myID >= id && id >= midID)) || (myID < midID && (myID >= id || midID < id))) {
		  // search the lower half of the ring
			for(int i = ftsize - 1; i >= (m-1); i--){
		...
		} else{  //search the upper half of the ring
			for(int i = m - 1; i >= 0; i--){
				int succID = fingertable[i].successor.nodeID;
				if((myID < id && (succID > myID && succID < id)) || (myID >= id && (succID > myID || succID < id))){
					return fingertable[i].successor;
	...
	// n here is the successor.
	// n will initialize our finger table.
	public void init_finger_table(NodeInfo n, Result result) throws RemoteException {
		...
		try {	
			...
			// Finger table initialization couldn't complete.
			// Stabilize an fix fingers will take care of it eventually.		
		}
	...
	*/
	...
	public void send_beat() throws RemoteException{
		//dummy function used to check node's running status
		//will throw 
	...
	*/
	...
	//Move keys in range (predecessor, n) to successor
