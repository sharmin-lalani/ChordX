...
tar -zcvf chordx.tar.gz *.java *.jar
...
scp chordx.tar.gz root@$i:/root/chordx.tar.gz > /dev/null 2>&1
ssh root@$i  "iptables -F"
ssh root@$i  "rm -rf /root/chordx> /dev/null 2>&1"
ssh root@$i  "pkill rmiregistry"
ssh root@$i  "pkill java"
ssh root@$i  "pkill java"
ssh root@$i  "mkdir /root/chordx"
ssh root@$i  "tar -zxvf chordx.tar.gz -C /root/chordx > /dev/null 2>&1"
ssh root@$i  "echo 'cd /root/chordx'>run.sh"
...
ssh root@$i  "echo 'java -cp .:log4j-1.2.17.jar ChordNodeImpl $i $bootstrapip </dev/null >/var/log/root-backup.log 2>&1 &'>>run.sh"
